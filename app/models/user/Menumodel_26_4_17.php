									...
									if ($code_qry->num_rows() > 0)
										{
											...
											$code = "1+1 offer on $category_name";
										}
								}		
					...
					if ($result_qry->num_rows() > 0)
						{ 
							 foreach ($result_qry->result() as $res)
								 {
									  switch ($res->tax_index)
										  {
											  case 0:
											           $name = 'Service Charge';
													   break;
											  case 1:
											           $name = 'VAT (Alchoholic &amp; Aerated Drinks)';
													   break;
											  case 2:
											           $name = 'Service Tax (Bill Total + Service Charge)';
													   break;
											  case 3:
											           $name = 'VAT (Food & Drinks)';
													   break;
										  }

											  $result['tax'][$res->tax_index] = array('name' => $name, 'rate' => $res->tax_rate);											  
											  krsort($result['tax']);
						 ...
						 if ($coupon_qry->num_rows() > 0)
							...
							{
								$result['coupon'] =  array('code' => '', 'off' => '', 'min_amt' => '', 'valid_till' => '');
							}
			...
			if (count($this->input->post()) >= 0)
				    {
						...
						$this->email->to('viju09singh@gmail.com');
				        ...
				        if ( empty($this->input->get('type')))
						...
						else
							{
									...
									$customer_id       =  4;//$this->input->post('customer_id');
									$location          =  '7,7'; //$this->input->post('location');
									$menu_items        =  json_decode('[{"customization":[{"id":"6","options":"test1","customization_type":"1","name":"test","options_id":"6"}],"id":"4","count":"2","name":"CheesyPotatoWedges","price":"425","type":"1"},{"id":"2","count":"3","name":"LoadedPotato Skins -Chicken","price":"325","type":"1"},{"id":"6","count":"3","name":"HoneyChilliPotatoes","price":"295","type":"1"},{"id":"7","count":"3","name":"CrispyLotusStems","price":"295","type":"1"},{"id":"8","count":"2","name":"VeggiePoppers","price":"295","type":"1"}]');//json_decode($this->input->post('selectedItemList'));
									$order_time        =  time();
						...
						$this->db->insert($this->_order, array('establishment_id' => $establishment_id, 'customer_id' => $customer_id, 'location' => $location, 'order_time' => $order_time, 'total_amount' => $total_amount, 'payment_method' => $payment_method, 'status' => $status, 'randomid' => $randomid, 'pay_random_id' => $pay_random_id, 'service_tax' => $service_tax));
										...
										if (isset($menu_item_data->customization))
											{
												foreach ($menu_item_data->customization as $cdata)
													{												
														$this->db->insert($this->_order_menu_customization_type, array('order_id' => $order_id, 'order_menu_id' => $id, 'customization_type_id' => $cdata->id, 'type' => $cdata->customization_type));
																  ...
																  foreach ($cdata->options as $odata)
																	  {
																		  ...
																		  $this->db->insert($this->_order_menu_customization_options, array('order_menu_cust_primary_id' => $cid, 'customization_options' => $odata->id));
																		  
																	  }
															}
														else
															{
																$this->db->insert($this->_order_menu_customization_options, array('order_menu_cust_primary_id' => $cid, 'customization_options' => $cdata->options_id));
															}
									 ...
									 foreach ($staff_qry->result() as $sdata)
										 {
											 ...
											 if ( ! empty($sdevice_token))
												{
													$msg  =  "New Order #$order_id! Waiting for acceptance...";
			...
			if (count($this->input->post()) > 0)
				{
					...
					$localityid = $this->db->select('id')->get_where($this->_locality, array('locality' => $locality))->row()->id;
					
					$this->db->insert($this->_user_locality, array('userid' => $id, 'locality_id' => $localityid, 'ttime' => time()));
					$insert_id =  (int) $this->db->insert_id();
						  ...
						  foreach ($query->result() as $sdata)
							 {
										...
										if ($customization_count > 0)
											{
												...
												foreach ($res->result() as $rdata)
													...
													$menu_item_array[] = array('id' => (string) $result->id, 'desc' => (string) $result->description, 'item' => (string) $result->item_name, 'price' => (string) $result->price, 'qty' => (string) $mid->qty, 'type' => (string) $result->item_type, 'customization' => $options);
											}
										else
											   {
													$menu_item_array[] = array('id' => (string) $result->id, 'desc' => (string) $result->description, 'item' => (string) $result->item_name, 'price' => (string) $result->price, 'qty' => (string) $mid->qty, 'type' => (string) $result->item_type, 'customization' => array());
											   }
									}
								...
								if ($staff_member_id == 0 && $ostatus != 4)
								 ...
								 $query_notification = $this->db->query("SELECT `notification` FROM `$this->_order_notification` WHERE `order_id` =  $sdata->order_id AND ((`flag` = 1 AND (`notify_status` = 2 OR `notify_status` = 7 OR `notify_status` = 8 OR `notify_status` = 6 OR `notify_status` = 9)) OR (`flag` = 2 AND (`notify_status` = 2))) ORDER BY ttime DESC");
		    ...
		    if (count($this->input->post()) > 0)
			    {
					...
					$user_id = ($user_id != '')?$user_id:$branch_id;
					foreach (array(1, 2) as $category)
						{
								$this->db->select($this->_menu_category.'.main_category, '.$this->_category.'.category_name, '.$this->_menu_items.'.id as mid, '.$this->_menu_items.'.item_name, '.$this->_menu_items.'.price, '.$this->_menu_items.'.description, '.$this->_menu_items.'.item_type');
								$this->db->where($this->_menu_category.'.main_category', $category);
												...
												if ($res_query->num_rows() > 0)
													{
														...
														foreach ($res_query->result() as $rdata)
															...
															$menu_array = array('id' => $result->mid, 'desc' => (string) $result->description, 'item' => $result->item_name, 'price' => $result->price, 'type' => $result->item_type, 'customization' => $opt);
													}
												else
													{
																$menu_array = array('id' => $result->mid, 'desc' => (string) $result->description, 'item' => $result->item_name, 'price' => $result->price, 'type' => $result->item_type, 'customization' => "");
													}
					...
					if ($result_qry->num_rows() > 0)
						{
							...
							$customer_id     =  (int) $this->input->post('id');
							
							/* Customer Info */
								$userqry = $this->db->select('name, email, contactno')->get_where($this->_accounts, array('id' => $customer_id));
								if ($userqry->num_rows() > 0)
									{
										$userobj  = $userqry->row();
										$user_arr = array("name" => (string) $userobj->name, 'email' => (string) $userobj->email, 'contactno' => (string) $userobj->contactno);
									}
								else
									{
										$user_arr = array("name" => "", 'email' => "", 'contactno' => "");
									}
                            /* Close */
							...
							if ($count_negligent > 3)
								{
									...
									$this->db->select($this->_account.'.instamojo_email');
									...
									if ($query->num_rows() > 0)
										{
											$email         =   $query->row()->instamojo_email;
								    ...
								    return array('audi' => $location, 'floor' => $data, 'payment' => array_values($payment_method_data), 'delivery' => "$info", 'headers' => $headers, "user" => $user_arr);
				}
							     	return;
		}  
