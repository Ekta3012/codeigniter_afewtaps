										 ...
										 if ($order_status != 5)
										  {
											  ...
											  require_once (APPPATH . 'class' . DIRECTORY_SEPARATOR . 'Gcm.php');
											  $gcm  =  new Gcm();
											  $msg  =  "This is a reminder from the customer for Order #$orderid. Please keep it under priority.";
					 ...
					 if ($result->num_rows() > 0)
						 {
							 ...
							 require_once (APPPATH . 'class' . DIRECTORY_SEPARATOR . 'Gcm.php');
							 $gcm  =  new Gcm();
							 $msg  =  "New Order #$orderid Accepted! Upon delivery, please mark the order as \"Complete\"";
		    ...
		    if (count($this->input->post()) > 0)
			    {
						...
						$this->db->select('order_id, customer_id, location, total_amount, order_time, payment_method, status, randomid, order_accept_time');
								...
								foreach ($query->result() as $sdata)
									{
										...
										$info[]   =   array('orderid' => $sdata->order_id, 'payment_method' => $payment, 'name' => $customer_info->name, 'location' => getOrderLocationAndroid($sdata->location), 'otime' => date('h:i a, d M Y', $sdata->order_time), 'price' => $sdata->total_amount, 'menu' => $menu_item_array, 'status' => $sdata->status, 'notification' => $order_res, 'comments' => $comments, 'flag' => $flag, 'ttime' => $ttime);
									}
								...
								foreach ($query->result() as $sdata)
									{
										...
										switch ($payment_method)
										    ...
										    $info[]   =    array('orderid' => $sdata->order_id, 'payment_method' => $payment, 'name' => $customer_info->name, 'location' => getOrderLocationAndroid($sdata->location), 'otime' => date('h:i a, d M Y', $sdata->order_time), 'price' => $sdata->total_amount, 'menu' => $menu_item_array, 'status' => $sdata->status, 'comments' => $comments, 'flag' => $sdata->new_flag_service_app, 'escalated' => $sdata->escalation_flag_service_app);
