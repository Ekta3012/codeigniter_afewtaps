						  ...
						  foreach ($query->result() as $sdata)
							 {
								...
								if ($staff_member_id == 0 && $ostatus != 4)
								 ...
								 $query_notification = $this->db->query("SELECT `notification` FROM `$this->_order_notification` WHERE `order_id` =  $sdata->order_id AND `flag` = 1 AND (`notify_status` = 2 OR `notify_status` = 7) ORDER BY ttime ASC");
								 ...
								 $info[]   =   array('orderid' => $sdata->order_id, 'payment_method' => $payment, 'location' => $sdata->location, 'otime' => date('h:i a, d M Y', $sdata->order_time), 'price' => $sdata->total_amount, 'menu' => $menu_item_array, 'notification' => $order_res, 'sname' => $sname, 'status' => $status, 'comments' => $comments, 'del_intimation' => $sdata->delivery_intimation);    												
							 }									
					    }
				}
                                return $info;
