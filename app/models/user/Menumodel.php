...
class Menumodel extends CI_Model {
  ...
  public function codPaymentModule() {
    if (count($this->input->post()) >= 0) {
      if (empty($this->input->get('type'))) {
        ...
        $establishment_id = 1; //$this->input->post('establishment_id');
  ...
  public function orderModule() {
    if (count($this->input->post()) >= 0) {
      ...
      $location = $this->input->post('location');
      $menu_items = json_decode($this->input->post('selectedItemList'));
      $order_accept_time = $order_time = time();
      $total_amount = $this->input->post('total_amount');
      $so_order_time = $this->input->post('so_order_time');
      $pay_method = $this->input->post('payment_method');
      $payment_method = (!empty($pay_method)) ? $pay_method : 2;
      ...
      if ($staff_member_id > '0') {
        $added_by = '2'; //Added By Staff
      } else {
        $added_by = '1'; //Added By Customer
      }
      //} else {
      //$establishment_id = 1; //$this->input->post('establishment_id');
      //$customer_id = 1; //$this->input->post('customer_id');
      ...
      $this->db->insert($this->_order, array('establishment_id' => $establishment_id, 'customer_id' => $customer_id, 'location' => $location, 'order_time' => $order_time, 'so_order_time' => $so_order_time, 'order_accept_time' => $order_accept_time, 'total_amount' => $total_amount, 'staff_member_id' => $staff_member_id, 'payment_method' => $payment_method, 'status' => $status, 'randomid' => $randomid, 'pay_random_id' => $pay_random_id, 'added_by' => $added_by));
      $order_id = $this->db->insert_id();
      ...
      foreach ($menu_items as $menu_item_data) {
        $id = $menu_item_data->id;
        $quantity = $menu_item_data->count;
        $comment = $menu_item_data->comment;

        $this->db->insert($this->_order_menu_id, array('order_id' => $order_id, 'menu_id' => $id, 'qty' => $quantity));

        // Order Menu Comment
        $this->db->insert($this->_order_menu_comment, array('order_id' => $order_id, 'order_menu_id' => $id, 'comment' => $comment));
        // End here
      ...
      if ($staff_qry->num_rows() > 0) {
        ...
        foreach ($staff_qry->result() as $sdata) {
          $sdevice_token = $sdata->device_token;
          if (!empty($sdevice_token)) {
            $msg = "Order #$order_id! In preparation...&orderid=$order_id";
            $gcm->sendNotification($sdevice_token, $msg);
          }
        }
      }
      ...
      return array('order_id' => $order_id, 'locality' => "$locality", "total_amount" => $total_amount);
    }
    return 0;
  }
