	...
	public function forgotPasswordModule()
		{
			...
			$this->db->where('email', $email)->update($this->_account, array('password' => sha1($random)));
			
			$this->load->library('email');
			
			$this->email->from('no-reply@afewtaps.com', 'afewtaps');
			$this->email->to($email);
			$this->email->subject('afewtaps - establishment reset password');
			$this->email->message('Hi, Your afewtaps establishment password is . '. $random);
			$this->email->send();
			 ...
			 if ($query->num_rows() > 0)
				 {
					  ...
					  /* Instamojo Configuration */
						...
						$instaemail   =   $result->instamojo_email;
						
						if (empty($instaemail))
							{
								$this->db->where('id', $id)->update($this->_account, array('instamojo_email' => $email));
