...
class Menumodel extends CI_Model {
			...
			if ( ! $this->upload->do_upload('userfile'))
			...
			else
				{
					...
					while ($x <= $excel->sheets[0]['numRows']) 
						{	   
							   $category_name 				= 	trim($excel->sheets[0]['cells'][$x][1]);
							   $sub_category_name   		= 	trim($excel->sheets[0]['cells'][$x][2]);
							   $item_name 		    		= 	trim($excel->sheets[0]['cells'][$x][3]);
							   $item_price          		= 	trim($excel->sheets[0]['cells'][$x][4]);
							   $description         		= 	trim($excel->sheets[0]['cells'][$x][5]);
							   $item_type           		= 	trim($excel->sheets[0]['cells'][$x][6]);
							   $customization_name          = 	trim($excel->sheets[0]['cells'][$x][7]);
							   $customization_type          = 	trim($excel->sheets[0]['cells'][$x][8]);
	...
	*/
	...
	public function delMenu($menu_id = '', $user_id = '')
		{	  
			  $category_id = $this->db->select('category_id')->get_where($this->_menu_category, array('menu_id' => $menu_id))->row()->category_id;
			  
			  $num_rows    = (int) $this->db->get_where($this->_menu_category, array('category_id' => $category_id))->num_rows();
			  
			  if ($num_rows == 1)
				  {
					  $this->db->where('id', $category_id)->delete($this->_category);
				  }
			  
			  $this->db->where('menu_id', $menu_id)->delete($this->_branch_menu);
			  $this->db->where('id', $menu_id)->delete($this->_menu_items);
			  $this->db->where('menu_id', $menu_id)->delete($this->_menu_category);
			  
			  $custom = $this->db->select('id')->get_where($this->_menu_customization_type, array('menu_id' => $menu_id));
			  if ($custom->num_rows() > 0)
				  {
					  foreach ($custom->result() as $res)
                      $typeid[] = $res->id;
					  
					  //$this->db->where('menu_id', $menu_id)->delete($this->_menu_customization_type);
					  //$this->db->where_in('customization_type_id', $typeid)->delete($this->_menu_customization_options);
				  } 
