...
class Ordermodel extends CI_Model {
    ...
    public function getOrderHistoryDetails($establid = '',$added_by=NULL) {
        
        $curr_date = date('Y-m-d');
        
        $first_day_month = strtotime($curr_date);
        $month_name = date('M', $first_day_month);
        $month_numeric = date('m', $first_day_month);
...
//        die();
        ...
        $this->db->select('order_id, customer_id, location, total_amount, order_time, payment_method, status, completion_time, cancel_time, staff_member_id, new_order_flag, pending_order_flag, user_nudge,added_by');
        $this->db->order_by('order_id','Desc');
        if ($added_by != "") {
            $this->db->where($this->_order.'.added_by', $added_by);
        }
...
//        echo $this->db->last_query();die();
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $sdata) {
                ...
                $staff_info = $this->db->get_where($this->_staff_member, array('id' => $sdata->staff_member_id))->row_array();
                ...
                $order[] = array('orderid' => $sdata->order_id, 'payment_method' => $payment, 'name' => $customer_info->name, 'location' => $sdata->location, 'otime' => date('h:i a, d F Y', $sdata->order_time), 'price' => $sdata->total_amount, 'count' => count($arr), 'orders' => $arr, 'time_diff' => $time_diff, 'new_order_flag' => 2, 'sname' => '', 'user_nudge' => 0, 'cid' => $sdata->customer_id, 'stfid' => $sdata->staff_member_id,'staff_name'=>$staff_name,'added_by'=>$sdata->added_by);
            }
        }
...
//        die();
    ...
    public function getOrderHistoryDetails_staff($establid = '',$added_by=NULL) {
...
//        echo $this->db->last_query();die();
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $sdata) {
                ...
                $order[] = array('orderid' => $sdata->order_id, 'payment_method' => $payment, 'name' => $customer_info->name, 'location' => $sdata->location, 'otime' => date('h:i a, d F Y', $sdata->order_time), 'price' => $sdata->total_amount, 'count' => count($arr), 'orders' => $arr, 'time_diff' => $time_diff, 'new_order_flag' => 2, 'sname' => '', 'user_nudge' => 0, 'cid' => $sdata->customer_id, 'stfid' => $sdata->staff_member_id,'staff_name'=>$staff_name);
            }
        }
//        echo $this->db->last_query();
        return $order;
    ...
    public function getOrderHistoryDetails_old($establid = '', $added_by = NULL) {
...
//        echo $this->db->last_query();die();
    ...
    public function getOrders_staff($estabid = '',$added_by=NULL) {
        ...
        $this->db->select($this->_staff_member . '.name as sname, ' . $this->_order . '.order_id, ' . $this->_order . '.order_time, ' . $this->_order . '.status, ' . $this->_order . '.total_amount, ' . $this->_order . '.payment_method, ' . $this->_order . '.total_amount, ' . $this->_order . '.location');
        $this->db->order_by('order_id','Desc');
        if ($added_by != "") {
            $this->db->where($this->_order.'.added_by', $added_by);
        }
...
//        die();
    ...
    public function getOrders($estabid = '',$added_by=NULL) {
        $curr_date = date('Y-m-d');
...
//        die();
        ...
        $this->db->select($this->_accounts . '.name as sname, '.$this->_staff_member . '.name as staff_name,' . $this->_order . '.order_id, ' . $this->_order . '.order_time, ' . $this->_order . '.status, ' . $this->_order . '.total_amount, ' . $this->_order . '.payment_method, ' . $this->_order . '.total_amount, ' . $this->_order . '.location,'. $this->_order . '.added_by');
        $this->db->order_by('order_id','Desc');
...
//            $etime = mktime(23, 59, 59, date('m'), date('d'), date('Y'))
        ...
        $this->db->from($this->_order);
        $this->db->join($this->_accounts, "$this->_accounts.id = $this->_order.customer_id",'left');
        $this->db->join($this->_staff_member, "$this->_staff_member.id = $this->_order.staff_member_id",'left');
        $data['result'] = $this->db->get()->result();
