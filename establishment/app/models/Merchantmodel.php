...
class Merchantmodel extends CI_Model {
	...
	public function getMerchantInfo($id = '')
		{
			return $this->db->get_where($this->_merchant_info, array('userid' => $id))->row();
		}
	...
	public function addUpdateMerchantInfo($id = '')
		{
			...
			if ($num_rows > 0)
			...
			else
			    {
					...
					$insta_email =  $this->db->select('instamojo_email')->get_where($this->_account, array('id' => $sess_id))->row()->instamojo_email;
					
					$query = $this->db->get_where($this->_instamojo, array('email' => $insta_email));
