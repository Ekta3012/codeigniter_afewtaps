...
function loadNotifications() {
		...
		if (hostname == 'localhost')
			var url = 'http://localhost/fewtaps/establishment/';
		else
			var url = 'http://www.afewtaps.com/establishment/';
		
		$.ajax({
					 url: url + "index.php/order/notification",
					 type:"GET",
					 dataType:"json",
					 ...
					 success:function(P) 
					   {
							var H = '';
							var L = parseInt(P.result.length);
							$("#notfiyCount").html(L); 
							if (L > 0)
								{
									$.each(P.result, function (V,R)  {
										
										H += '<div style="padding:2px 8px" id="notificationContainer" class="media-body" data-attr="' + R.is_read + '" style="margin:8px 0">' + R.notification +'<br /><small class="text-muted">' +  R.time + '</small></div>';
									});
								}
							else
								{
									H += '<div class="media-body">No Notification found!</div>';
								}
									$("#notifyContainer").html(H);
					   }
	          }); 
}