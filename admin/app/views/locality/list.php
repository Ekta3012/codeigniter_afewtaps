...
<body>
    ...
    padding: 0;">No. of Restaurant: <?php 	echo count($listofrestaurant);?></label><div class="col-sm-6" style="text-align:right;  font-weight: bold;"></div>
									</div>
								</div>
								<?php
										   ...
										   foreach ($listofrestaurant as $data)
											   {
												   ...
												   $merchantid = $this->db->select('userid')->get_where($this->db->dbprefix('merchant_estab'), array('estabid' => $data->{'id'}))->row()->userid;
												   $owner = $this->db->select('name')->get_where($this->db->dbprefix('account'), array('id' => $merchantid))->row()->name;
