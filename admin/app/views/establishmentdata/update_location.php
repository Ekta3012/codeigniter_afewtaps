      ...
      <div id="page-wrapper" class="gray-bg sidebar-content">
	   ...
	   <?php $this->load->view('include/inc_topnav'); ?>
        ...
        <div class="wrapper wrapper-content" id="leftWrapper">
		   ...
		   <?php echo form_open('establishmentdata/editAudi/'.$this->uri->segment(3)); ?>
								...
								<div class="bar brdrad5" style="border-bottom-left-radius:0;border-bottom-right-radius:0;">View Establishments</div>
								  <div class="ibox-content">
                                     ...
                                     <!-- <div class="row text-center">
									    ...
									    <div class="row">
											<div class="col-md-12">
												<?php 
													...
													if (count($branches) > 0)
													  {
														 echo '<select class="form-control m-b" name="estabid" onchange="branchLoc(this.value)">';
														 echo '<option value="">Select Establishment</option>';
														 foreach ($branches as $bdata)
														   {
															   $selected = ($bdata->id == $data['cinema']->cinema_id) ? "selected='selected'" : "" ;
										...
										if (is_object($cinema))
											{
												echo '<div class="row" id="cinemaContainer">
														<div class="row">
															<div class="col-lg-1"></div>
																<div class="col-lg-10 text-center xgs" id="audiCont">
																  <div class="row mrgntpbt">
															        <input type="text" name="audi" class="pull-left form-control wdt60" placeholder="Audi Name" autocomplete="off" value="'.$cinema->audi_id.'" />
														</div>';
 
											    $res = $this->db->get_where($this->db->dbprefix('cinema_rows'), array('cinema_audi_id' => $cinema->id));
												if ($res->num_rows() > 0)
													{
														$button = 0;
														$f = 0;
														foreach ($res->result() as $res_data)
															{
																$row_count++;
																echo '<div class="row xvp" id="rowd">
																		   <div class="row mrgn"> 
																			 <input type="text" name="rowno" class="pull-left form-control wdt60"  placeholder="Enter Row No." value="'.$res_data->row_no.'" />';
																			...
																			$result = $this->db->get_where($this->db->dbprefix('cinema_seats'), array('cinema_rows_id' => $res_data->id));
																			if ($result->num_rows() > 0)
																				{
																					$btn = 0;
																					foreach ($result->result() as $result_data)
																						{
																							echo '<div class="row mrgncgb"><input type="text" name="audi_seats[1]['.$result_data->id.']" class="pull-left form-control wdt60" placeholder="Enter Seat No" autocomplete="off" value='.$result_data->seats_no.' />';
																							
																							if ($btn == 0)
																							echo '<span class="addMore" data-start='.$f.' data-end=\'1\' id="seatAddContainer">&nbsp;</span><span class="addRemove" id="seatRemContainer">&nbsp;</span>';
...
?>
												...
												<div class="row">
												  <div class="col-lg-1"></div>
													<div class="col-lg-10 text-center xgs" id="audiCont">
														...
														<div class="row mrgntpbt">
															<input type="text" name="audi[0][name]" class="pull-left form-control wdt60" placeholder="Audi Name" autocomplete="off">
														</div>
														...
														<div class="row xvp" id="rowd">
														   ...
														   <div class="row mrgncgb"> 
															<input type="text" name="audi[0][row_no][0][seat_no][]" class="pull-left form-control wdt60" placeholder="Enter Seat No" autocomplete="off" />
						...
						<input type="hidden" id="audiCount" name="audiCount" value="<?php echo $row_count; ?>" />
        ...
        $(document).ready(function(){
            ...
            $('.dataTables-example').DataTable({
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excel', title: 'establishment'},
                    {extend: 'pdf', title: 'establishment'},
	...
	$("body").on("click", "#seatAddContainer", function() {
	  ...
	  var P = '';
	  P += '<div class="row mrgncgb"><input name="audi_seats[0][]" type="text" placeholder="Enter Seat No" class="pull-left form-control wdt60" name="seatAddContainer"></div>';
